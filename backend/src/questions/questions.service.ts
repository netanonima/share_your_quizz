...
export class QuestionsService {
  ...
  async update(id: number, updateQuestionDto: UpdateQuestionDto, user: User) {
    ...
    if (updateQuestionDto.media && updateQuestionDto.mediaName) {
      if (question.media) {
        await this.mediaService.eraseFile(
          question.media.file_path,
          question.media.filename,
          question.media.extension,
        );
        await this.mediaRepository.remove(question.media);
      }

      const media = new Media();
      const mimeType = updateQuestionDto.media.match(
        /[^:]\w+\/[\w-+\d.]+(?=;|,)/,
      )[0];
      ...
      let thisExtension = '';
      if (type === 'image') {
        newBuffer = await this.mediaService.convertAndResizeImage(
          updateQuestionDto.media,
        );
        await this.mediaService.writeBufferToFile(
          newBuffer,
          filePath,
          newMediaName,
        );
      ...
      if (type === 'audio' || mimeType.split('/')[1] == 'ogg') {
        duration = await this.mediaService.saveFile(
          updateQuestionDto.media,
          filePath + filename + '.' + oldExtension,
        );
        size =
          Math.round((updateQuestionDto.media.length / 1024 / 1024) * 100) /
          100;
        const currentAudioFilePath = filePath + filename + '.' + oldExtension;
        filename = filename + '1';
        const newAudioFilePath = filePath + filename + '.' + newExtension;
        await this.mediaService.convertFile(
