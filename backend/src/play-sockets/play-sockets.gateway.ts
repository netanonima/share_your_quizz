...
export class PlaySocketsGateway {
  ...
  handleAdminJoin(
    @MessageBody() data: AdminJoinInterface,
    ...
    if (!this.sessions.has(sessionId)) {
      this.sessions.set(sessionId, {
        ...
        questionsRanking: [],
        answersDistribution: [],
        params: { shuffle_questions: false, shuffle_choices: false },
      });
    }
  ...
  handleIsReady(
    @MessageBody() data: IsReadyInterface,
    @ConnectedSocket() client: Socket,
  ): void {
  ...
  handleJoin(
    @MessageBody() data: JoinInterface,
    @ConnectedSocket() client: Socket,
  ): void {
  ...
  async handleGameLaunch(
    @MessageBody() data: GameLaunchInterface,
    @ConnectedSocket() client: Socket,
  ): Promise<void> {
    ...
    if (session.params.shuffle_questions) {
      ...
      session.questions = thisQuizz.questions.sort((a, b) =>
        a.id > b.id ? 1 : -1,
      );
    }
    ...
    for (let i = 0; i < session.questions.length; i++) {
      if (session.questions[i].media) {
        let mediaPath =
          session.questions[i].media.file_path +
          session.questions[i].media.filename +
          '.' +
          session.questions[i].media.extension;
        // manage backslashes
        ...
        fs.readFile(mediaPath, (error, mediaData) => {
          ...
          if (session.questions[i].media.extension.toLowerCase() === 'mp3')
            realExtension = 'mpeg';
          if (session.questions[i].media.extension.toLowerCase() === 'jpg')
            realExtension = 'jpeg';

          session.questions[i].media =
            "data:'" +
            session.questions[i].media.type +
            '/' +
            realExtension +
            "';base64," +
            mediaBase64;
    ...
    function endingAction() {
      for (let i = 0; i < session.questions.length; i++) {
        ...
        const choices = session.questions[i].choices;
        const choicesList = [];
      ...
      session.ranking = {
        players: session.users.map((user) => ({
  ...
  handleNextQuestion(
    @MessageBody() data: NextQuestionInterface,
    @ConnectedSocket() client: Socket,
  ): void {
    ...
    if (session.params.shuffle_choices) {
      ...
      const currentAdminQuestionChoices = [];
      currentQuestion.choices.forEach((choice) => {
        const currentChoice = currentAdminQuestion.choices.find(
          (currentChoice) => currentChoice.choiceId === choice.choiceId,
        );
      ...
      const currentQuestionAnswersDistribution = [];
      currentQuestion.choices.forEach((choice) => {
        const currentChoice = session.answersDistribution[session.current].find(
          (currentChoice) => currentChoice.id === choice.choiceId,
        );
        currentQuestionAnswersDistribution.push(currentChoice);
      });
      session.answersDistribution[session.current] =
        currentQuestionAnswersDistribution;
    } else {
      currentQuestion.choices = currentQuestion.choices.sort((a, b) =>
        a.choiceId > b.choiceId ? 1 : -1,
      );
      currentAdminQuestion.choices = currentAdminQuestion.choices.sort((a, b) =>
        a.choiceId > b.choiceId ? 1 : -1,
      );
    }
  ...
  handleAnswer(
    @MessageBody() data: AnswerInterface,
    @ConnectedSocket() client: Socket,
  ): void {
    ...
    const currentQuestionAnswersDistribution =
      session.answersDistribution[session.current];
    const choiceIndex = currentQuestionAnswersDistribution.findIndex(
    ...
    if (isCorrect) {
      const thisQuestionScore = Math.round(
        1000 -
          (currentDatetime.getTime() -
            currentQuestion.askingDatetime.getTime()) /
            100,
      );
      if (
        session.current === undefined ||
        session.current < 0 ||
        session.current >= session.questionsRanking.length
      ) {
        console.error('Invalid session.current:', session.current);
        return;
      }
      if (
        Array.isArray(session.questionsRanking[session.current]) ||
        !session.questionsRanking[session.current]
      ) {
        session.questionsRanking[session.current] = { players: [] };
      }
      const questionRanking = session.questionsRanking[session.current];
      const playerIndex = questionRanking.players.findIndex(
        (player) => player.username === username,
      );
      if (playerIndex !== -1) {
        questionRanking.players[playerIndex].currentScore += thisQuestionScore;
      } else {
        questionRanking.players.push({
          username: username,
          currentScore: thisQuestionScore,
        });
      }

      session.questionsRanking[session.current] = questionRanking;

      const rankingPlayerIndex = session.ranking.players.findIndex(
        (player) => player.username === username,
      );
      if (rankingPlayerIndex !== -1) {
        session.ranking.players[rankingPlayerIndex].currentScore +=
          thisQuestionScore;
      } else {
        session.ranking.players.push({
          username: username,
          currentScore: thisQuestionScore,
        });
      }
  ...
  handleStopQuestion(
    @MessageBody() data: StopQuestionInterface,
    @ConnectedSocket() client: Socket,
  ): void {
    const { sessionId } = data;
    const session = this.sessions.get(sessionId);
    ...
    session.users.forEach((user) => {
      const username = user.username;
      const currentQuestionPoints = questionRanking.players.find(
        (player) => player.username === username,
      ).currentScore;
      let isCorrect = false;
      if (currentQuestionPoints > 0) isCorrect = true;
      const currentQuestionRanking =
        questionRanking.players.findIndex(
          (player) => player.username === username,
        ) + 1;
      const userResult = {
        isCorrect: isCorrect,
    ...
    if (isLastQuestion) {
      ...
      this.sessionsService.update(sessionId, {
  ...
  handleCurrentGlobalRankingTrigger(
    ...
    session.users.forEach((user) => {
      const username = user.username;
      const globalPoints = ranking.players.find(
        (player) => player.username === username,
      ).currentScore;
      const globalRanking =
        ranking.players.findIndex((player) => player.username === username) + 1;
      const userResult = {
  ...
  handlePlayerReconnectTry(
    @MessageBody() data: ReconnectInterface,
    @ConnectedSocket() client: Socket,
  ): void {
    ...
    const userIndex = session.oldUsers.findIndex(
      (user) => user.user_id === user_id,
    );
    if (userIndex !== -1) {
      const username = session.oldUsers[userIndex].username;
      const activePlayerWithSameUsername = session.users.find(
      ...
      const thisUserIndex = session.users.findIndex(
        (user) => user.username === username,
      );
